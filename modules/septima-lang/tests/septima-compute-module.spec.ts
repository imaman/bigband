import { failMe } from '../src/fail-me'
...
describe('septima-compute-module', () => {
   ...
   */
  const run = (mainFileName: string, inputs: Record<string, string>, args: Record<string, unknown> = {}) => {
  ...
  test.only('can use exported definitions from another module', () => {
  ...
  test('the args object is available only at the main module', () => {
    expect(() =>  run('a', { a: `import * as b from 'b'; args.x + '_' + b.foo`, b: `let foo = args.x ?? 'N/A'; {}` }, { x: 'Red' }),
