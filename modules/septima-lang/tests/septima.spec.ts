...
describe('septima', () => {
  ...
  test.only('error message specifies the location in the file', () => {
  ...
  describe('array methods', () => {
    ...
    test.only('findIndex returns -1 if no matching element exists', () => {
  ...
  describe('undefined', () => {
    ...
    describe('??', () => {
      test.only('if the lhs is undefined evaluates to the rhs', () => {
        expect(run(`undefined ?? 42`)).toEqual(42)
        expect(run(`undefined ?? 900`)).toEqual(900)
        expect(run(`undefined ?? 'Luke'`)).toEqual('Luke')
      })
      test.only('if the lhs is not undefined evaluates to the lhs', () => {
  ...
  describe('throw', () => {
    test.only('it raises an error that is propagated all the way out', () => {
      expect(() => run(`throw "bo" + "om"`)).toThrowError(
        `boom when evaluating:\n` + `  at (<inline>:1:8..16) bo" + "om`,
      )
    })
    test.only('the error message contains a septima stack trace that reflects the entire call chain', () => {
      expect(() =>
